...
{
    Task<ClienteResponse> ExecutarAsync(int id);
}

public sealed class ConsultarClienteUseCase(IClienteRepository repository)
    : ClienteUseCaseBase(repository), IConsultarClienteUseCase
{
    public async Task<ClienteResponse> ExecutarAsync(int id)
    {
