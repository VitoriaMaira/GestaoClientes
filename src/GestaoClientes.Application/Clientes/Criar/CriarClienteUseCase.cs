...
{
    Task<ClienteResponse> ExecutarAsync(CriarClienteRequest request);
}

public sealed class CriarClienteUseCase(
    IClienteRepository repository,
    IUnitOfWork unitOfWork) : ClienteUseCaseBase(repository), ICriarClienteUseCase
{
    public async Task<ClienteResponse> ExecutarAsync(CriarClienteRequest request)
    {
        await VerificarUnicidade(request.Cpf, request.Email);
        ...
        var cliente = new Cliente(
            ...
            ClienteMapper.Map(request.Endereco));
