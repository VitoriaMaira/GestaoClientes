...
public sealed class ClienteService(IClienteRepository repo, IUnitOfWork unitOfWork)
{
 public async Task<ClienteResponse> CriarAsync(CriarClienteRequest r){ await VerificarUnicos(r.Cpf,r.Email); var c=new Cliente(r.Nome,r.Cpf,r.Email,r.Telefone,r.DataNascimento,End(r.Endereco)); await repo.AdicionarAsync(c); await unitOfWork.SalvarAsync(); return Map(c); }
 ...
 public async Task AtualizarAsync(int id,AtualizarClienteRequest r){var c=await Obter(id);await VerificarUnicos(r.Cpf,r.Email,id);c.AtualizarDados(r.Nome,r.Cpf,r.Email,r.Telefone,r.DataNascimento);await unitOfWork.SalvarAsync();}
 public async Task InativarAsync(int id){(await Obter(id)).Inativar();await unitOfWork.SalvarAsync();} public async Task AtivarAsync(int id){(await Obter(id)).Ativar();await unitOfWork.SalvarAsync();}
 public async Task<IReadOnlyCollection<EnderecoResponse>> EnderecosAsync(int id)=>(await Obter(id)).Enderecos.Select(Map).ToList();
 public async Task AdicionarEnderecoAsync(int id,EnderecoRequest r){var c=await Obter(id);c.AdicionarEndereco(End(r));await unitOfWork.SalvarAsync();}
 public async Task AtualizarEnderecoAsync(int id,int enderecoId,EnderecoRequest r){var c=await Obter(id);c.ObterEndereco(enderecoId).Atualizar(r.Cep,r.Logradouro,r.Numero,r.Complemento,r.Bairro,r.Cidade,r.Estado,r.Principal);if(r.Principal)c.DefinirEnderecoPrincipal(enderecoId);await unitOfWork.SalvarAsync();}
 public async Task RemoverEnderecoAsync(int id,int enderecoId){(await Obter(id)).RemoverEndereco(enderecoId);await unitOfWork.SalvarAsync();} public async Task PrincipalAsync(int id,int enderecoId){(await Obter(id)).DefinirEnderecoPrincipal(enderecoId);await unitOfWork.SalvarAsync();}
