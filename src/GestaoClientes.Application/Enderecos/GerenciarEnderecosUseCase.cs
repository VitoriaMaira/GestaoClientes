...
public sealed class GerenciarEnderecosUseCase(IClienteRepository repository,IUnitOfWork unitOfWork) : ClienteUseCaseBase(repository), IGerenciarEnderecosUseCase
{
    ...
    public async Task AtualizarAsync(int id,int enderecoId,EnderecoRequest r){var c=await ObterCliente(id);c.ObterEndereco(enderecoId).Atualizar(r.Cep,r.Logradouro,r.Numero,r.Complemento,r.Bairro,r.Cidade,r.Estado,r.Principal);if(r.Principal)c.DefinirEnderecoPrincipal(enderecoId);await unitOfWork.SalvarAsync();}
