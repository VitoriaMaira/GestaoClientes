...
public sealed class Endereco
{
    ...
    public void Atualizar(string cep,string logradouro,string numero,string? complemento,string bairro,string cidade,string estado,bool principal) { Cep=SomenteDigitos(cep,"CEP",8); Logradouro=Obrigatorio(logradouro,"Logradouro"); Numero=Obrigatorio(numero,"Número"); Complemento=complemento?.Trim(); Bairro=Obrigatorio(bairro,"Bairro"); Cidade=Obrigatorio(cidade,"Cidade"); Estado=Obrigatorio(estado,"Estado").ToUpperInvariant();if(Estado.Length!=2)throw new DomainException("Estado deve possuir 2 caracteres."); Principal=principal; }
    ...
    private static string SomenteDigitos(string valor,string campo,int tamanho){var numero=new string(Obrigatorio(valor,campo).Where(char.IsDigit).ToArray());return numero.Length==tamanho?numero:throw new DomainException($"{campo} inválido.");}
}
