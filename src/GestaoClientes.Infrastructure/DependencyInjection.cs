...
public static class DependencyInjection
{
    ...
    {
        var connection = configuration.GetConnectionString("SqlServer") ?? throw new InvalidOperationException("A conexão 'SqlServer' não foi configurada.");
        services.AddDbContext<GestaoClientesDbContext>(options => options.UseSqlServer(connection));
        services.AddScoped<IClienteRepository, ClienteRepository>(); services.AddScoped<IUnitOfWork, UnitOfWork>();
        return services;
    }
}
