...
public class CriarClienteUseCaseTests
{
    ...
    private sealed class RepositorioFalso:IClienteRepository,IUnitOfWork
    {
        ...
        public Task<bool> CpfExisteAsync(string cpf,int? ignorarId=null)=>Task.FromResult(CpfExiste);public Task<bool> EmailExisteAsync(string email,int? ignorarId=null)=>Task.FromResult(EmailExiste);
        public Task<Cliente?> ObterAsync(int id)=>Task.FromResult<Cliente?>(null);public Task<(IReadOnlyList<Cliente> Itens,int Total)> ListarAsync(int pagina,int tamanho,string? nome,string? cpf,StatusCliente? status)=>Task.FromResult<(IReadOnlyList<Cliente>,int)>(([],0));
